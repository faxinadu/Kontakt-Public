--[[ 
Rename File Note Names to Numbers
...
for index, file in pairs(sample_paths_table) do
    local match_number
    local match_found
    for i=1,#note_names_table do 
        match_found = string.find (file, note_names_table[i])
        if (match_found and match_found > 0) then
            match_number = i
        end
    end
    local new_name = string.gsub(file,note_names_table[match_number],match_number-1)
